...
* **BirthdayLuxury (Sinh Nhật Hoàng Gia)**: Tông màu đen - vàng gold quý phái, vương miện xoay 3D, nút 🥂 *Khai tiệc Champagne* sủi bọt khí bay lên và **Sổ Vàng Lời Chúc VIP** để ghi nhận lời chúc của khách mời.
...
```
*(Nếu không điền MONGO_URI hoặc kết nối lỗi, hệ thống sẽ tự động dùng JSON Database dự phòng)*
...
npm run dev
```
*(Frontend sẽ chạy ở cổng `http://localhost:5173` và tự động kết nối proxy API tới backend ở cổng `5000`)*
